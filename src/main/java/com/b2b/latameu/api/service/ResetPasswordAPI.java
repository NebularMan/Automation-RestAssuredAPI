...
//        apiUrl = getlatamServerUrl()+ LoadProperties.readCommonConfigProperty("resetPassword");
//        LoadProperties.logger.info("Reset Password API URL :- " + apiUrl);
    }
    ...
    public Response getResetPasswordResponse(HashMap<String, Object> testdata, HashMap<String, Object> resMap) throws Exception {
            ...
            switch (testField){
                ...
                case "newPassword":
                    if(testVal.equalsIgnoreCase("ValidValue")) {
                        ...
                        updatelatamConfigValue("newPassword",newpassword);
                        System.out.println("Updated new password is "+LoadProperties.getLatamColumbiaPropertyValue("newPassword"));
        ...
        apiUrl = apiServer+ readCommonConfigProperty("resetPassword");
        headerMap = getSiteIdMap(getProject().toString(),headerMap);
        headerMap = getXapiKeyMap(getProject().toString(),headerMap);
        System.out.println("Reset password API URL :- " + apiUrl);
        System.out.println("Reset password Header :- " + headerMap);
        System.out.println("Reset password Header :- " + bodyMap);

        Response response = BaseApi.postAPI(bodyMap, headerMap,apiUrl);
        LoadProperties.logger.info("Resend Email or Forgot Password API response is :- " + response);
        return response;
    }
