...
public class ServiceAPI extends Basecode {
    ...
    public HashMap<String, Object> getRequestHeaderBodyTestcaseData(String endpoint, String testcase) throws Exception {
        ...
        System.out.println("Value from environment variable :"+project);
        if(project.equalsIgnoreCase("WhatsApp")){
            path = usrdir+ LoadProperties.getLatamColumbiaPropertyValue("whatsAppTestDataPath");
        }else{
            path = usrdir+ LoadProperties.getCommonConfigPropertyValue(project+"TestDataPath");
        }
        System.out.println("Value of test data path : "+path);
    ...
    public HashMap<String, Object> getResponseDataFromTestDataFile(String endpoint, String testcase) throws Exception {
        ...
        if(getProject().equalsIgnoreCase("WhatsApp")){
            path = usrdir+ LoadProperties.getLatamColumbiaPropertyValue("whatsAppTestDataPath");
        }else{
            path = usrdir+ readCommonConfigProperty(getProject()+"TestDataPath");
        }
        sheet = getSheetObject(path,endpoint);
        int rowCount = getExcelRowCount(path,endpoint);
        int colCount = getExcelColumnCount(path,endpoint);
