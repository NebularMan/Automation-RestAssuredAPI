...
public class JoyServiceAPI extends Basecode {
    private ScenarioContext context;
    private Sheet sheet;
    private XSSFWorkbook wb;

    public JoyServiceAPI(ScenarioContext context) throws Exception {
    ...
    public HashMap<String, Object> getRequestHeaderBodyTestcaseData(String endpoint, String testcase) throws Exception {
        ...
        if(Project.equalsIgnoreCase("WhatsApp")){
            path = usrdir+ LoadProperties.getjoyLoyaltyPropertyValue("whatsAppTestDataPath");
        }else{
            path = usrdir+ LoadProperties.getjoyLoyaltyPropertyValue("testDataPath");
        }
    ...
    public HashMap<String, Object> getResponseDataFromTestDataFile(String endpoint, String testcase) throws Exception {
        ...
        String usrdir = System.getProperty("user.dir");
        String Project = getjoyLoyaltyPropertyValue("Project");
        if(Project.equalsIgnoreCase("WhatsApp")){
            path = usrdir+ LoadProperties.getjoyLoyaltyPropertyValue("whatsAppTestDataPath");
        }else{
            path = usrdir+ LoadProperties.getjoyLoyaltyPropertyValue("testDataPath");
    ...
    public HashMap<String, Object> getResponseTestDataFile(String endpoint, String testcase) throws Exception {
        HashMap<String, Object> dataMap = new HashMap<>();
        String path = null;
        String testId,outputField,outputValues,KeyValues;
        String usrdir = System.getProperty("user.dir");
        String Project = getjoyLoyaltyPropertyValue("Project");
        path = usrdir+ LoadProperties.getjoyLoyaltyPropertyValue("testDataPath");
        sheet = getSheetObject(path,endpoint);
        ...
        for(int i=2;i<rowCount;i++){
            ...
            int j = 1;
            testId = sheet.getRow(i).getCell(j).toString();
            KeyValues = sheet.getRow(i).getCell(j+5).toString();
            String[] keyArr = KeyValues.split(";");
            System.out.println(testId);
            if(testId.equalsIgnoreCase(testcase)){
                dataMap.put("keys" ,keyArr);
            }
        }
        System.out.println("Expected response body fields and values :"+dataMap);
        return dataMap;
